...
contract Redistribution is AccessControl, Pausable {
    ...
    * @dev Pause the contract. The contract is provably stopped by renouncing
    ...
    function commit(bytes32 _obfuscatedHash, bytes32 _overlay, uint256 _roundNumber) external whenNotPaused {
        ...
        //look into passing this require later
        //require(
    ...
    for (uint256 i = 0; i < currentRevealHashes.length; i++) {
        delete currentRevealToStake[currentRevealHashes[i]]; // Reset the value to 0
    }
    ...
    function reveal(bytes32 _overlay, uint8 _depth, bytes32 _hash, bytes32 _revealNonce) external whenNotPaused {
        ...
        for (uint256 i = 0; i < commitsArrayLength; i++) {
            if (currentCommits[i].overlay == _overlay && commitHash == currentCommits[i].obfuscatedHash) {
                //consider wether i Should bother with this check //get around this with 0 depth
                require(
               ...
               if (currentRevealToStake[_hash] == 0) {
                    currentRevealToStake[_hash] = currentCommits[i].stake * uint256(2 ** _depth);
                } else {
                    currentRevealToStake[_hash] += currentCommits[i].stake * uint256(2 ** _depth);
                }
                
                //emit stakeSetToHash(currentRevealToStake[ currentRevealHashes[i]]);
                currentReveals.push(
                    Reveal({
    ...
    function isWinner(bytes32 _overlay) public view returns (bool) {
        ...
        //new
        uint256 currentMaxReveal = 0;

        //can remove truth selection testing gas cost
        for (uint256 i = 0; i < commitsArrayLength; i++) {
    ...
    function claim() external whenNotPaused {
       ...
       //currentSum += bankStakeDensity;
        //create dummy reveal for bank
        emit maxStakeEmitted(currentMaxReveal,bankStakeDensity);
        ...
        for (uint256 i = 0; i < commitsArrayLength+1; i++) {
            ...
            //new
            if (i >= commitsArrayLength){
                randomNumber = keccak256(abi.encodePacked(winnerSelectionAnchor, k+1));
                randomNumberTrunc = uint256(randomNumber & MaxH);
    ...
    function setCurrentClaimRound(uint256 newRound) external {
        require(hasRole(PAUSER_ROLE, msg.sender), "only pauser can pause");

        currentClaimRound = newRound;
    }
     function getCurrentClaimRound() public view returns (uint256) {
        return currentClaimRound;
    }

    event maxStakeEmitted(uint256 indexed mStake, uint256 indexed bStake);
